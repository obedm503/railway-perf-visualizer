...
const SelectTrigger = <T extends ValidComponent = "button">(
  ...
  return (
    <SelectPrimitive.Trigger
      class={cn(
        "ring-offset-background placeholder:text-muted-foreground focus:ring-ring flex h-10 w-full cursor-pointer items-center justify-between rounded-md bg-transparent px-3 py-2 text-sm focus:ring-2 focus:ring-offset-2 focus:outline-none disabled:cursor-not-allowed disabled:opacity-50",
        local.class,
      )}
      {...others}
    >
  ...
  return (
    <SelectPrimitive.Portal>
      <SelectPrimitive.Content
        class={cn(
          "border-border bg-secondary-background text-foreground animate-in fade-in-80 relative z-50 min-w-32 overflow-hidden rounded-md border shadow-md",
          local.class,
        )}
        {...others}
      >
...
const SelectItem = <T extends ValidComponent = "li">(
  ...
  return (
    <SelectPrimitive.Item
      class={cn(
        "focus:bg-muted focus:text-foreground relative mt-0 flex w-full cursor-default items-center rounded-sm py-1.5 pr-8 pl-2 text-sm outline-none select-none data-disabled:pointer-events-none data-disabled:opacity-50",
        local.class,
      )}
      {...others}
    >
